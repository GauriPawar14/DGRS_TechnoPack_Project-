...
import com.example.corrugatedbox.project.Services.SizeService;
...
public class SizeController {
			...
			@PostMapping("/add")
			public ResponseEntity<?> registerdUser(@RequestBody Size size) {
				Size createdUser = sizeService.create(size);
			...
			public ResponseEntity<?> getAllUserser() {
				try {
					return new ResponseEntity<>(sizeService.fetchAll(), HttpStatus.OK);
				} catch (Exception e) {
			...
			public ResponseEntity<?> getUserById(@PathVariable("userId") Integer id) {
				try {
					return  ResponseEntity.ok(sizeService.fetchById(id));
			...
			public ResponseEntity<?> updateuserById(@PathVariable("userId") Integer id,
					@RequestBody Size updateUser) {
				try {
					Size existingUser=sizeService.fetchById(id);
					
					return  ResponseEntity.ok(sizeService.update(updateUser, existingUser));
			...
			{ try {
				Size existingUser=sizeService.fetchById(id);
				return ResponseEntity.ok(sizeService.delete(existingUser));
			...
			public ResponseEntity<?> getUserByUserName(@PathVariable String name){
				try {
					...
					Size foundUser = sizeService.fetchUserByUserName(name);
					if(foundUser != null) {
