...
import com.example.corrugatedbox.project.Services.ProductService;
...
public class ProductController {
				...
				@PostMapping("/add")
				public ResponseEntity<?> registerdUser(@RequestBody Product product) {
					Product createdUser = productService.create(product);
				...
				public ResponseEntity<?> updateuserById(@PathVariable("userId") Integer id,
						@RequestBody Product updateUser) {
					try {
						Product existingUser=productService.fetchById(id);
						
						return  ResponseEntity.ok(productService.update(updateUser, existingUser));
				...
				{ try {
					Product existingUser=productService.fetchById(id);
					return ResponseEntity.ok(productService.delete(existingUser));
							
				} catch (Exception e) {

					ErrorResponse errorResponce= new ErrorResponse("product deletion is failead", e.getMessage());
					return new ResponseEntity<>(errorResponce,HttpStatus.BAD_REQUEST);
				...
				public ResponseEntity<?> getUserByUserName(@PathVariable String name){
					try {
						...
						Size foundUser = productService.fetchUserByUserName(name);
						if(foundUser != null) {
